...
from multiview_detector.models.mvdet_ae_wild import PerspTransDetector2
from multiview_detector.models.mvdet_ae_multi import PerspTransDetector
...
def main(args):
    ...
    if 'wildtrack' in args.dataset:
        train_set = frameDataset(base, train=True, transform=train_trans, grid_reduce=4, img_reduce=8)
        test_set = frameDataset(base, train=False, transform=train_trans, grid_reduce=4, img_reduce=8)
    elif 'multiviewx' in args.dataset:
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=args.lr, steps_per_epoch=len(train_loader),
                                                    pct_start=0.15, epochs=args.epochs)
    ...
    if args.resume is None:
        ...
        for epoch in tqdm.tqdm(range(1, args.epochs + 1)):
            ...
            if moda > bestone:
                bestone = moda
                imname = f'MultiviewDetector.pth'
                torch.save(model.state_dict(), os.path.join(logdir, imname))
            print(print(f'max moda:{bestone}'))
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, default='wildtrack', choices=['wildtrack', 'multiviewx', 'terrace'])
    ...
    parser.add_argument('--data_path', type=str, default=None, help='数据集根目录，默认 ./Data/{dataset_name}')
